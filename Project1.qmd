...
# 2) Coerce types before filter
for (v in c("PWGTP","AGEP")) if (v %in% names(df)) {
...
# 3)  checks to see if there is valid data after filters 
sum(is.finite(df$AGEP))           # how many valid ages?
sum(is.finite(df$PWGTP))          # how many valid weights?
table(df$SEX, useNA = "ifany")    # levels present?

# 4) filter to produce an small feasible data
df_small <- dplyr::filter(df, is.finite(AGEP), AGEP >= 25, AGEP <= 65)

# 5) Keep class and run
class(df_small) <- c("census", class(df_small))
summary(df_small)
plot(df_small, num_var = "AGEP", cat_var = "SEX")
